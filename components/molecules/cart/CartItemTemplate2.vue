...
<template>
  <div class="flex bg-white pd-xs border-default rd-xl flex-1">
    <div class="width-100">
      <img :src="props.item.idProduct.image" :alt="props.item.idProduct?.productName" />
    </div>
    <div class="flex-1 pd-xs pl-sm">
      ...
      <div class="text-color-gray5 text-size-xs">
        <div v-for="itemV in item.selectedOptionsPush" :key="itemV.optionName">
          {{ itemV.optionName }}: <span>{{ itemV.variantName }}</span>
        </div>
        <div class="text-color-gray5 text-size-xs">
          {{ item.note }}
        </div>
      ...
      <div class="flex justify-between align-end mt-xs">
        ...
        <Text :text="`${formatCurrency(item.finalPriceDiscounts ? item.finalPriceDiscounts : item.priceDiscounts)}`" color="gray5" />
      </div>
    </div>
  </div>
</template>