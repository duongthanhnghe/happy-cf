...
<Popup popupId="popup-change-password" v-model="storeAccountEdit.isTogglePopupChangePassword" :popupHeading="AUTH_TEXT_CHANGE_PASSWORD" align="right">
  <template #body>
    <v-form validate-on="submit lazy" @submit.prevent="handleSubmitUpdate">
      ...
      <v-text-field :append-icon="storeAccountEdit.showPassword ? 'mdi-eye' : 'mdi-eye-off'" :type="storeAccountEdit.showPassword ? 'text' : 'password'" @click:append="storeAccountEdit.showPassword = !storeAccountEdit.showPassword" v-model="storeAccountEdit.newPassword" :rules="newPasswordRules" :label="AUTH_TEXT_PASSWORD_HINT" autocomplete="new-password" variant="outlined" required></v-text-field>
      <LabelInput :label="AUTH_TEXT_CONFIRM_PASSWORD" required/>
      <v-text-field :append-icon="storeAccountEdit.showPasswordConfirm ? 'mdi-eye' : 'mdi-eye-off'" :type="storeAccountEdit.showPasswordConfirm ? 'text' : 'password'" @click:append="storeAccountEdit.showPasswordConfirm = !storeAccountEdit.showPasswordConfirm" v-model="storeAccountEdit.newPasswordConfirm" :rules="confirmPasswordRules" :label="AUTH_TEXT_PASSWORD_HINT" autocomplete="new-password" variant="outlined" required></v-text-field>
