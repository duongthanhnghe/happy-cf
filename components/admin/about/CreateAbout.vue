...
import { createAboutSchema } from '@/shared/validate/schemas/about.schema'

const store = useAboutManageStore();
const editorRef = ref()
const { validate, formErrors } = useValidate(createAboutSchema)
...
<template>
  ...
  >
    ...
    <template #footer>
      <Button @click="handleSubmitCreate()" color="primary" label="Lưu bài viết" class="w-full" />
    </template>
  </Popup>
</template>
