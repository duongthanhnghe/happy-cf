...
export const useAdminPostAll = () => {
  ...
  const fetchPostList = async (page: number, limit: number, search: string, categoryId:string) => {
    try {
      const data: PostNewsPaginationDTO = await newsAPI.getAllPosts(page, limit, search, categoryId)
