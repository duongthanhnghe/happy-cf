...
definePageMeta({
  layout: ROUTES.ADMIN.IMAGE_BLOCK.layout,
  middleware: ROUTES.ADMIN.IMAGE_BLOCK.middleware,
})
    ...
    <v-data-table-server 
      ...
      <template #item.page="{ item }">
        <v-chip label color="blue">
          {{
            IMAGE_BLOCK_PAGE_OPTIONS.find(p => p.value === item.page)?.title
              || item.page
          }}
        </v-chip>
      </template>
      ...
      <template #item.position="{ item }">
        <v-chip label>
          {{
            IMAGE_BLOCK_POSITION_OPTIONS.find(p => p.value === item.position)?.title
              || item.position
          }}
        </v-chip>
      </template>
